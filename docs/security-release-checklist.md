...
| Tray with 1, 20, and 1000 rows | Apple Silicon and Intel | The default tray shows all retained rows; configured limits show exactly the newest requested rows; each row copies directly; text hover shows a line-preserving bounded side preview; rebuilding never reads rich detail | Menu count, hover/click result, and timing summary     |
...
directory under macOS's per-user temporary area; the real ClipEcho database
...
| Private storage and defaults         | Isolated data directory was `0700`; SQLite database was `0600`; schema version was 2. Item limit was 100, byte limit 256 MiB, compact mode and restore-to-top were off, and no ClipEcho LaunchAgent existed before user interaction.                                          |
